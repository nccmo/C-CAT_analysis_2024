...
## 2.2 COSMIC data 
```{r}
df <- df_mutations_F1CDx_withVEPannovar_variant
make_cosmic <- function(df){
  ...
  # count cosmic number
  m <- stringr::str_split(df_cos$cosmic70list, "\\(.*?\\),",  simplify = TRUE)  #leaves numbers
  m_int <- matrix(as.integer(m), nrow = nrow(m), ncol = ncol(m))
  df_cos$cosmic70count <- rowSums(m_int, na.rm = TRUE) #takes sum
...
# df_F1CDx_SNPfilter_withrescue %>% 
  # filter(VAF_calc < 0.05 & depth != 0) %>% nrow()


# previously, depth == 0 were removed. 
# current data, some samples have VAF data but no altread/depth data. for those mutations, the original VAF is used and mutations < 0.05 are removed.
df_F1CDx_SNPfilter_withrescue %>% 
  filter(VAF_calc < 0.05) %>% 
  nrow()
# 9810
df_F1CDx_SNPfilter_withrescue %>% 
  filter(VAF_calc < 0.05 & !is.na(depth)) %>% nrow()
df_F1CDx_SNPfilter_withrescue %>% 
  filter(VAF_calc < 0.05 & is.na(depth)) %>% nrow()

df_NCC_SNPfilter_withrescue %>% 
  filter(VAF_calc < 0.05 & depth != 0) %>% nrow()
...
df_mutations_F1CDx_VAFcutoff_prevariantfilter %>% 
  write_tsv("result/2_ccat/table/1_panel_selection/2_mutation_sheet/4_mutfiltering/F1CDx_VEPannovar_SNPVAF_withsilent.txt")

df_mutations_F1CDx_VAFcutoff_prevariantfilter %>% 
  count(Variant_Classification)

df_mutations_NCC_VAFcutoff_prevariantfilter %>% 
  write_tsv("result/2_ccat/table/1_panel_selection/2_mutation_sheet/4_mutfiltering/NCC_VEPannovar_SNPVAF_withsilent.txt")
...
# 6.0 Save
```{r}
# df_sample_F1CDx_withVAF %>% 
#   write_tsv("result/2_ccat/table/1_panel_selection/1_sample_sheet/F1CDx_sample_VAF.txt")
# df_sample_NCC_withVAF %>% 
#   write_tsv("result/2_ccat/table/1_panel_selection/1_samplesheet/NCC_sample_VAF.txt")
...
df_mutations_F1CDx_VAFcutoff %>% nrow()
df_mutations_NCC_VAFcutoff %>% nrow()
```

# 7.0 SNP check
## 7.1 TP53
```{r}
df_mutations_F1CDx_withVEPannovar_correctinfo %>% filter(Hugo_Symbol == "TP53") %>%
...
df_mutations_F1CDx_withVEPannovar_cos %>% filter(Hugo_Symbol == "TP53") %>%
  filter(HGVSp_Short %in% c("p.A189V", "p.D49H", "p.E11Q", "p.E458V", "p.V31I", "p.N263D")) %>%
  distinct(Hugo_Symbol, HGVSp_Short, esp6500siv2_all, `1000g2014oct_all`, gnomad211exome_AF_popmax, ToMMo14K_AF, togovar_AF, cosmic70count) 

#   Hugo_Symbol HGVSp_Short esp6500siv2_all `1000g2014oct_all` gnomad211exome_AF_popmax ToMMo14K_AF togovar_AF cosmic70count
#   <chr>       <chr>                 <dbl>              <dbl>                    <dbl>       <dbl>      <dbl>         <dbl>
# 1 TP53        p.N263D                  NA           0.000200 *               0.0008  *     NA         NA                   2
# 2 TP53        p.A189V                  NA          NA                       0.0003  *      0.00219*  0.00186*             4
# 3 TP53        p.D49H                   NA          NA                       0.0000544     0.0023*     0.00161*             4
# 4 TP53        p.E11Q                   NA           0.000200  *              0.0007 *       0.00747*    0.00684*             9
# "p.A189V"*, "p.D49H*", "p.E11Q*", "p.N263D"*
# "p.E458V", "p.V31I" → these two not included

# c.91G>A
df_mutations_F1CDx_withVEPannovar_cos %>% 
  filter(Hugo_Symbol == "TP53", HGVSp)

df_mutations_F1CDx_withVEPannovar_cos %>% 
  # select(AAChange.refGene) %>% 
  filter(Hugo_Symbol == "TP53", str_detect(AAChange.refGene, "p.V31")) %>%
  # filter(Hugo_Symbol == "TP53", str_detect(AAChange.refGene, "p.E458")) %>% 
  count(Hugo_Symbol, HGVSp_Short, AAChange.refGene)

# Chr17:7579705

# none
df_mutations_F1CDx_withVEPannovar_cos %>% 
  filter(Chromosome == "17", Start_Position == 7579705) %>% 
  count(Hugo_Symbol, HGVSp_Short, AAChange.refGene)
...
# df_DFCI_withVEPannovar_pre %>% filter(Hugo_Symbol == "TP53") %>%
#   filter(HGVSp_Short %in% c("p.A189V", "p.D49H", "p.E11Q", "p.E458V", "p.V31I", "p.N263D")) %>%
#   count(Hugo_Symbol, HGVSp_Short, esp6500siv2_all, `1000g2014oct_all`, gnomad211exome_AF_popmax, ToMMo14K_AF, togovar_AF, cosmic70) 
# # TP53        p.E11Q                   NA           0.000200                   0.0007     0.00747    0.00684
# 
# df_MSK_withVEPannovar_pre %>% filter(Hugo_Symbol == "TP53") %>%
#   filter(HGVSp_Short %in% c("p.A189V", "p.D49H", "p.E11Q", "p.E458V", "p.V31I", "p.N263D")) %>%
#   count(Hugo_Symbol, HGVSp_Short, esp6500siv2_all, `1000g2014oct_all`, gnomad211exome_AF_popmax, ToMMo14K_AF, togovar_AF, cosmic70) 
# 
# df_DUKE_withVEPannovar_pre %>% filter(Hugo_Symbol == "TP53") %>%
#   filter(HGVSp_Short %in% c("p.A189V", "p.D49H", "p.E11Q", "p.E458V", "p.V31I", "p.N263D")) %>%
#   count(Hugo_Symbol, HGVSp_Short, esp6500siv2_all, `1000g2014oct_all`, gnomad211exome_AF_popmax, ToMMo14K_AF, togovar_AF, cosmic70) 

df_mutations_F1CDx_VAFcutoff %>% filter(Hugo_Symbol == "TP53") %>%
  filter(HGVSp_Short %in% c("p.A189V", "p.D49H", "p.E11Q", "p.E458V", "p.V31I", "p.N263D")) %>%
  count(HGVSp_Short,
        # ONCOTREE_CODE
        ) %>%
...
df_mutations_NCC_VAFcutoff %>% filter(Hugo_Symbol == "TP53") %>%
  filter(HGVSp_Short %in% c("p.A189V", "p.D49H", "p.E11Q", "p.E458V", "p.V31I", "p.N263D")) %>%
  count(HGVSp_Short,
        # ONCOTREE_CODE
        ) %>%
